...
var line = d3.svg.line()
    ...
    .y(function(d) { return y(d["percent_paid"]); });
...
data.forEach(function(d){
    ...
    d["percent_paid"] = +d["percent_paid"];
});

x.domain(d3.extent(data, function(d) { return d["start_date"]; }));
y.domain([0, d3.max(data, function(d) { return d["percent_paid"]; })]);
...
focus.attr("transform", "translate(" + x(d["start_date"]) + "," + y(d["percent_paid"]) + ")");
focus.select("text").text(d["percent_paid"]);
}
